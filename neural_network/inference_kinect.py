...
def inference_one_view(rgb_file, depth_file, meta_file, scene_idx, anno_idx):
    ...
    fx = 607.4124755859375
    fy = 607.33538818359375
    cx = 637.793212890625
    cy = 365.12252807617188
    ...
    if anno_idx < 5 and FLAGS.save_visu:
        ...
        final_dir = os.path.join(SAVE_PATH, split, 'scene_%04d'%scene_idx, camera, 'visu')
        ...
        # final_image=cv2.imread('0001_final.png',cv2.IMREAD_COLOR)   
...
def inference(scene_idx):
    ...
    meta_file = os.path.join(dataset_root, '0000.mat')

    final_file = inference_one_view(rgb_file, depth_file, meta_file, scene_idx, anno_idx)
